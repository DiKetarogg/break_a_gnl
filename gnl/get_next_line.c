...
/*   Updated: 2022/01/18 03:22:38 by jzoltan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	list_init(t_list *l, int fd, size_t app_size)
{
	t_list	list;
...
t_list	list_get_node(t_list node, int fd)
{
	if (!node)
		return (0);
	while (node->next != 0)
...
void	list_pop(t_list *root, t_list node)
{
	if (node->root)
	{
...
int	get_next_line_internal(t_list node)
{
	int	r;
	...
	while (r > 0)
	{
		...
		{
			if (!ft_string_app_char(node->string, node->buffer[node->b_index]))
				return (0);
